...
function Header({ cartItemCount, onLoginClick, onSignUpClick, isSignedUp }) {
  return (
    <header className="header">
      <div className="logo">Product Sales Website</div>
...
function App() {
  ...
  const handleLoginSubmit = (e) => {
    ...
    if (loginForm.username === signUpForm.username && loginForm.password === signUpForm.password) {
      ...
      setAuthError('Authentication Failed, cannot Login'); // Set error message
    }
  };
  ...
  const handleSignUpSubmit = (e) => {
    ...
    if (signUpForm.password !== signUpForm.confirmPassword) {
      setAuthError('Passwords do not match'); // Set error message
      return;
    }
    ...
    emailjs.send(SERVICE_ID, TEMPLATE_ID, templateParams, USER_ID)
      .then(response => {
        ...
        setAuthError(''); // Clear any previous errors
        setIsSignedUp(true);
        setShowSignUpModal(false); // Close modal after successful sign-up
      })
      .catch(error => {
        console.error('Error sending email:', error);
        setAuthError('Failed to send confirmation email'); // Set error message
      });
  };
